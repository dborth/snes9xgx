/****************************************************************************
 * Snes9x 1.51 Nintendo Wii/Gamecube Port
 *
 * Tantric February 2009
 *
 * gui_keyboard.cpp
 *
 * GUI class definitions
 ***************************************************************************/

#include "gui.h"
/**
 * Constructor for the GuiKeyboard class.
 */

GuiKeyboard::GuiKeyboard(char * t)
{
	width = 540;
	height = 400;
	shift = 0;
	caps = 0;
	selectable = true;
	focus = 0; // allow focus
	alignmentHor = ALIGN_CENTRE;
	alignmentVert = ALIGN_TOP;
	strncpy(kbtextstr, t, 100);
	kbtextstr[100] = 0;

	Key thekeys[4][10] = {
	{
		{'1','!'},
		{'2','@'},
		{'3','#'},
		{'4','$'},
		{'5','%'},
		{'6','^'},
		{'7','&'},
		{'8','*'},
		{'9','('},
		{'0',')'}
	},
	{
		{'q','Q'},
		{'w','W'},
		{'e','E'},
		{'r','R'},
		{'t','T'},
		{'y','Y'},
		{'u','U'},
		{'i','I'},
		{'o','O'},
		{'p','P'}
	},
	{
		{'a','A'},
		{'s','S'},
		{'d','D'},
		{'f','F'},
		{'g','G'},
		{'h','H'},
		{'j','J'},
		{'k','K'},
		{'l','L'},
		{':',';'}
	},

	{
		{'z','Z'},
		{'x','X'},
		{'c','C'},
		{'v','V'},
		{'b','B'},
		{'n','N'},
		{'m','M'},
		{',','<'},
		{'.','>'},
		{'/','?'}
	}
	};
	memcpy(keys, thekeys, sizeof(thekeys));

	keyTextbox = new GuiImageData(keyboard_textbox_png);
	keyTextboxImg = new GuiImage(keyTextbox);
	keyTextboxImg->SetAlignment(ALIGN_CENTRE, ALIGN_TOP);
	keyTextboxImg->SetPosition(0, 20);
	keyTextboxImg->SetParent(this);

	kbText = new GuiText(kbtextstr, 22, (GXColor){0, 0, 0, 0xff});
	kbText->SetAlignment(ALIGN_CENTRE, ALIGN_TOP);
	kbText->SetPosition(0, 30);
	kbText->SetParent(this);

	keyLarge = new GuiImageData(keyboard_largekey_png);
	keyLargeOver = new GuiImageData(keyboard_largekey_over_png);

	key = new GuiImageData(keyboard_key_png);
	keyOver = new GuiImageData(keyboard_key_over_png);
	keySoundOver = new GuiSound(button_over_mp3, button_over_mp3_size);
	trigA = new GuiTrigger;
	trigA->SetSimpleTrigger(-1, WPAD_BUTTON_A | WPAD_CLASSIC_BUTTON_A, PAD_BUTTON_A);

	keyBackImg = new GuiImage(keyLarge);
	keyBackOverImg = new GuiImage(keyLargeOver);
	keyBackText = new GuiText("Back", 22, (GXColor){0, 0, 0, 0xff});
	keyBack = new GuiButton(keyLarge->GetWidth(), keyLarge->GetHeight());
	keyBack->SetImage(keyBackImg);
	keyBack->SetImageOver(keyBackOverImg);
	keyBack->SetLabel(keyBackText);
	keyBack->SetSoundOver(keySoundOver);
	keyBack->SetTrigger(trigA);
	keyBack->SetPosition(10*42+40, 0*42+80);
	keyBack->SetParent(this);

	keyCapsImg = new GuiImage(keyLarge);
	keyCapsOverImg = new GuiImage(keyLargeOver);
	keyCapsText = new GuiText("Caps", 22, (GXColor){0, 0, 0, 0xff});
	keyCaps = new GuiButton(keyLarge->GetWidth(), keyLarge->GetHeight());
	keyCaps->SetImage(keyCapsImg);
	keyCaps->SetImageOver(keyCapsOverImg);
	keyCaps->SetLabel(keyCapsText);
	keyCaps->SetSoundOver(keySoundOver);
	keyCaps->SetTrigger(trigA);
	keyCaps->SetPosition(0, 2*42+80);
	keyCaps->SetParent(this);

	keyShiftImg = new GuiImage(keyLarge);
	keyShiftOverImg = new GuiImage(keyLargeOver);
	keyShiftText = new GuiText("Shift", 22, (GXColor){0, 0, 0, 0xff});
	keyShift = new GuiButton(keyLarge->GetWidth(), keyLarge->GetHeight());
	keyShift->SetImage(keyShiftImg);
	keyShift->SetImageOver(keyShiftOverImg);
	keyShift->SetLabel(keyShiftText);
	keyShift->SetSoundOver(keySoundOver);
	keyShift->SetTrigger(trigA);
	keyShift->SetPosition(21, 3*42+80);
	keyShift->SetParent(this);

	for(int i=0; i<4; i++)
	{
		for(int j=0; j<10; j++)
		{
			keyImg[i][j] = new GuiImage(key);
			keyImgOver[i][j] = new GuiImage(keyOver);
			keyTxt[i][j] = new GuiText(NULL, 22, (GXColor){0, 0, 0, 0xff});
			keyBtn[i][j] = new GuiButton(key->GetWidth(), key->GetHeight());
			keyBtn[i][j]->SetImage(keyImg[i][j]);
			keyBtn[i][j]->SetImageOver(keyImgOver[i][j]);
			keyBtn[i][j]->SetSoundOver(keySoundOver);
			keyBtn[i][j]->SetTrigger(trigA);
			keyBtn[i][j]->SetLabel(keyTxt[i][j]);
			keyBtn[i][j]->SetPosition(j*42+21*i+40, i*42+80);
			keyBtn[i][j]->SetParent(this);
		}
	}
}

/**
 * Destructor for the GuiKeyboard class.
 */
GuiKeyboard::~GuiKeyboard()
{
	delete keyTextbox;
	delete keyTextboxImg;
	delete key;
	delete keyOver;
	delete keySoundOver;
	delete trigA;

	for(int i=0; i<4; i++)
	{
		for(int j=0; j<10; j++)
		{
			delete keyImg[i][j];
			delete keyImgOver[i][j];
			delete keyTxt[i][j];
			delete keyBtn[i][j];
		}
	}
}

void GuiKeyboard::SetFocus(int f)
{
	focus = f;

	for(int i=0; i<4; i++)
	{
		for(int j=0; j<10; j++)
		{
			keyBtn[i][j]->ResetState();
		}
	}
}

void GuiKeyboard::ResetState()
{
	if(state != STATE_DISABLED)
		state = STATE_DEFAULT;

	for(int i=0; i<4; i++)
	{
		for(int j=0; j<10; j++)
		{
			keyBtn[i][j]->ResetState();
		}
	}
}

/**
 * Draw the keyboard on screen
 */
void GuiKeyboard::Draw()
{
	keyTextboxImg->Draw();
	kbText->Draw();

	char txt[2] = { 0, 0 };

	for(int i=0; i<4; i++)
	{
		for(int j=0; j<10; j++)
		{
			if(shift || caps)
				txt[0] = keys[i][j].chShift;
			else
				txt[0] = keys[i][j].ch;

			keyTxt[i][j]->SetText(txt);
			keyBtn[i][j]->Draw();
		}
	}

	keyCaps->Draw();
	keyShift->Draw();
	keyBack->Draw();
}

void GuiKeyboard::Update(GuiTrigger * t)
{
	keyCaps->Update(t);
	keyShift->Update(t);
	keyBack->Update(t);

	if(keyBack->GetState() == STATE_CLICKED)
	{
		kbtextstr[strlen(kbtextstr)-1] = 0;
		kbText->SetText(kbtextstr);
		keyBack->ResetState();
	}
	if(keyShift->GetState() == STATE_CLICKED)
	{
		shift ^= 1;
		keyShift->ResetState();
	}
	if(keyCaps->GetState() == STATE_CLICKED)
	{
		caps ^= 1;
		keyCaps->ResetState();
	}

	for(int i=0; i<4; i++)
	{
		for(int j=0; j<10; j++)
		{
			keyBtn[i][j]->Update(t);

			if(keyBtn[i][j]->GetState() == STATE_CLICKED)
			{
				if(shift || caps)
				{
					kbtextstr[strlen(kbtextstr)] = keys[i][j].chShift;
					if(shift) shift ^= 1;
				}
				else
					kbtextstr[strlen(kbtextstr)] = keys[i][j].ch;

				kbText->SetText(kbtextstr);
				keyBtn[i][j]->ResetState();
			}
		}
	}
}
