...
{
	return Settings.videoScanlines && videoDriver->getVideoMode()->efbHeight > 300;
}
...
void OgcEmulatorVideo::resetVideo()
{
	...
	if (Settings.videoMode == VIDEOMODE_ORIGINAL_240P)
	{
		for (int j=0; j<4; j++)
		{
	...
	{
		if (Settings.videoAspectRatioCorrection != VIDEO_ASPECT_RATIO_CORRECTION_NONE)
...
void OgcEmulatorVideo::presentFrame(int width, int height)
{
	...
	if (CheckVideo)	// if we get back from the menu, and have rendered at least 1 frame
	{
		...
		if (Settings.videoMode == VIDEOMODE_ORIGINAL_240P)
		{
			...
			if (Settings.videoAspectRatioCorrection == VIDEO_ASPECT_RATIO_CORRECTION_16_9 || Settings.videoAspectRatioCorrection == VIDEO_ASPECT_RATIO_CORRECTION_16_9_FIXED) {
				xscale = (3*xscale)/4;
			}
		}
		else
		{
			if (Settings.videoAspectRatioCorrection == VIDEO_ASPECT_RATIO_CORRECTION_16_9) {
			...
			else if (Settings.videoAspectRatioCorrection == VIDEO_ASPECT_RATIO_CORRECTION_16_9_FIXED) {
		...
		xscale *= Settings.videoZoomHor;
		yscale *= Settings.videoZoomVert;

		square[6] = square[3]  =  xscale + Settings.videoXshift;
		square[0] = square[9]  = -xscale + Settings.videoXshift;
		square[4] = square[1]  =  yscale - Settings.videoYshift;
		...
		gameScreenPng.xoffset = Settings.videoXshift * (videoDriver->getScreenWidth() / (float)menu_vmode->viWidth) * ((float)vmode->viWidth / (float)vmode->fbWidth);
		gameScreenPng.yoffset = Settings.videoYshift * (videoDriver->getScreenHeight() / menuViHeightAdjusted) * (viHeightAdjusted / (float)vmode->efbHeight);
