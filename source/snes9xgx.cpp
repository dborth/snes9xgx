...
int main(int argc, char *argv[])
{
	...
	while (platform->getSystemEvent() != SystemEvent::ShutdownRequested) // main loop
	{
		...
		Settings.Mute = Settings.MuteAudio;
		Settings.SupportHiRes = (Settings.HiResolution == 1);
		Settings.MaxSpriteTilesPerLine = (Settings.SpriteLimit ? 34 : 128);
		Settings.SkipFrames = (Settings.FrameSkip ? AUTO_FRAMERATE : 0);
		Settings.AutoDisplayMessages = (Settings.DisplayFrameRate || Settings.DisplayTime ? true : false);
		Settings.MultiPlayer5Master = (Settings.Controller == CTRL_PAD4 ? true : false);
		Settings.SuperScopeMaster = (Settings.Controller == CTRL_SCOPE ? true : false);
		Settings.MouseMaster = (Settings.Controller == CTRL_MOUSE || Settings.Controller == CTRL_MOUSE_PORT2 || Settings.Controller == CTRL_MOUSE_BOTH_PORTS);
		Settings.JustifierMaster = (Settings.Controller == CTRL_JUST ? true : false);
		SetControllers ();
...
void ExitApp() {
	...
	if (SNESROMSize > 0 && appRequest != AppRequest::MENU && Settings.AutoSave == AUTOSAVE_SRAM)
		SaveSRAMAuto(SILENT);
