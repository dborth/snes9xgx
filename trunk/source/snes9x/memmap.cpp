...
char *CMemory::SafeANK (const char *s)
{
    ...
    for (int i = 0; i < len; i++)
    {
		...
		if (Memory.ROMRegion == 0 && ((unsigned char) s[i] >= 0xa0 && (unsigned char) s[i] < 0xe0)) // JIS X 201
			safe [i] = s[i];
		else
			safe [i] = '?';
...
bool8 CMemory::Init ()
{
    ...
    if (!RAM || !SRAM || !VRAM || !ROM ||
        ...
        !IPPU.TileCache [TILE_2BIT_EVEN] || !IPPU.TileCache [TILE_2BIT_ODD] ||
        !IPPU.TileCache [TILE_4BIT_EVEN] || !IPPU.TileCache [TILE_4BIT_ODD] ||
        !IPPU.TileCached [TILE_2BIT_EVEN] || !IPPU.TileCached [TILE_2BIT_ODD] ||
        !IPPU.TileCached [TILE_4BIT_EVEN] || !IPPU.TileCached [TILE_4BIT_ODD])
    {
		Deinit ();
		return (FALSE);
    }
...
void CMemory::Deinit ()
{
	...
	for(int t=0; t<7; t++){
		if(IPPU.TileCache[t]) {
			free ((char *) IPPU.TileCache[t]);
			IPPU.TileCache[t] = NULL;
		}
		if(IPPU.TileCached[t]) {
			free ((char *) IPPU.TileCached[t]);
...
//MK: Damn. YI trips a BRK currently. Maybe even on a real cart.
		...
		if(strncmp((char *)&ROM[0x7fc0], "YOSHI'S ISLAND", 14) == 0 && READ_WORD(ROM+0x7FDE)==57611 && ROM[0x10002]==0xA9)
		{
			Interleaved=true;
...
uint32 CMemory::FileLoader (uint8* buffer, const char* filename, int32 maxsize)
{
	...
	default:
		...
		unsigned long ARAM_max = maxsize + 0x200 - (ptr - ROM);
		FileSize = ARAM_ROMSIZE;

		if ( FileSize > ARAM_max )
			FileSize = ARAM_max;

		if ( hasloaded == 0 )
			ARAMFetchSlow( (char *)ptr, (char *)AR_SNESROM, FileSize );
...
bool8 CMemory::LoadMulti (const char *basename, const char *slot1name, const char *slot2name)
{
	...
	if(0== TotalFileSize) return FALSE;

#ifndef NGC
	CheckForIPSPatch (basename, HeaderCount != 0, TotalFileSize);
#endif
	...
	if(slot1name[0]!='\0')
	{
		...
		if(0== TotalFileSize) return FALSE;
#ifndef NGC
		CheckForIPSPatch (slot1name, HeaderCount != 0, TotalFileSize);
#endif
	...
	if(slot2name[0]!='\0')
	{
		...
		if(0== TotalFileSize) return FALSE;
#ifndef NGC
		CheckForIPSPatch (slot2name, HeaderCount != 0, TotalFileSize);
#endif
...
void CMemory::InitROM (bool8 Interleaved)
{
	...
	{
		Map [MEMMAP_NUM_BLOCKS] = (uint8 *) MAP_NONE;
		BlockIsRAM [MEMMAP_NUM_BLOCKS] = FALSE;
		BlockIsROM [MEMMAP_NUM_BLOCKS] = FALSE;
	}
...
//			Settings.MultiPlayer5Master = FALSE;
	...
	if (strlen (ROMName))
	{
		char *p = ROMName + strlen (ROMName);
                if(p>ROMName+21 && ROMName[20]==' ') p=ROMName+21;
...
bool8 CMemory::LoadSRAM (const char *filename)
{
    ...
    if (size)
    {
		...
		if (Settings.BS && !Settings.BSXItself)
		{
			...
			{
				fread((char *) ::SRAM, 1, 0x20000, file);
				fclose (file);
...
[14:25:27] <@Nach>     case 0x340f23e5: //Donkey Kong Country 3 (U) copier hack - handled
	*/
	...
	if(strncmp(ROMName, "SD\x0b6\x0de\x0dd\x0c0\x0de\x0d1GX", 10)==0) // SD Gundam GX
	{
		//Set DSP-3
		DSP1.version=2;
		SetDSP = &DSP3SetByte;
...
#if 0
// These two games don't have SRAM. Sounds like OpenBus issue...
    ...
    if (strncmp (ROMName, "BATMAN--REVENGE JOKER", 21) == 0)
    {
		Memory.HiROM = FALSE;
		Memory.LoROM = TRUE;
		LoROMMap ();
	...
	// Force Disabling a speed-up hack (CPU_Shutdown())
    ...
    if (strcmp (ROMName, "EARTHWORM JIM 2") == 0 ||
		...
		strncmp (ROMId, "ARF", 3) == 0) // Star Ocean
    {
	...
	//CPU timing hacks
	Timings.H_Max       = (SNES_CYCLES_PER_SCANLINE * Settings.CyclesPercentage) / 100;
	Timings.HBlankStart = SNES_HBLANK_START_HC;
	Timings.HDMAStart   = SNES_HDMA_START_HC;
	Timings.RenderPos   = SNES_RENDER_START_HC;

	//The first new hack since timings were changed :(
	if (strncmp(ROMName, "SeikenDensetsu 2", 16) == 0 ||
		strncmp(ROMName, "Secret of MANA", 14) == 0)
	{
		Timings.HBlankStart -= 34;
		Timings.HDMAStart -= 34;
	}

/*	if(strcmp(ROMName, "HOME IMPROVEMENT")==0)
		Timings.H_Max = (SNES_CYCLES_PER_SCANLINE * 200) / 100;

    if (strcmp (ROMId, "ASRJ") == 0 && Settings.CyclesPercentage == 100)
		// Street Racer
		Timings.H_Max = (SNES_CYCLES_PER_SCANLINE * 95) / 100;

	// Power Rangers Fight
    if (strncmp (ROMId, "A3R", 3) == 0 ||
        // Clock Tower
		strncmp (ROMId, "AJE", 3) == 0)
		Timings.H_Max = (SNES_CYCLES_PER_SCANLINE * 103) / 100;

    if (strncmp (ROMId, "A3M", 3) == 0 && Settings.CyclesPercentage == 100)
		// Mortal Kombat 3. Fixes cut off speech sample
		Timings.H_Max = (SNES_CYCLES_PER_SCANLINE * 110) / 100;
    ...
    if (strcmp (ROMName, "\x0bd\x0da\x0b2\x0d4\x0b0\x0bd\x0de") == 0 &&
		...
		Timings.H_Max = (SNES_CYCLES_PER_SCANLINE * 101) / 100;
    // Start Trek: Deep Sleep 9
    if (strncmp (ROMId, "A9D", 3) == 0 && Settings.CyclesPercentage == 100)
		Timings.H_Max = (SNES_CYCLES_PER_SCANLINE * 110) / 100;
*/
	Timings.H_Max_Master = Timings.H_Max;
